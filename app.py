...
3. **図面ラベル差分抽出** (compare labels) - 2つのDXFファイルのラベルを比較し、差分をマークダウン形式で出力します
4. **Excel回路記号抽出** (extract symbols) - ULKES Excelファイルから回路記号を抽出します
5. **回路記号リスト差分抽出** (compare partslist) - 2つの回路記号リストを比較し、差分をマークダウン形式で出力します
...
st.markdown("### 使用方法")
st.markdown("""
各ツールページではファイルをアップロードし、必要なオプションを設定してから処理を実行します。
処理結果はダウンロード可能なファイルとして提供されます。
""")
